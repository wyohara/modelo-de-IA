...
def encoding_posicional(seq_len, dim_model):
    """
    Gera a matriz de codificação posicional senoidal.
    Retorna um array de formato (seq_len, d_model)
    """
    PE = np.zeros((seq_len, dim_model))

    for pos in range(seq_len):
        for i in range(0, dim_model, 2):
            # iterando a seq_len e dim_model
            angle = pos / (10000 ** (2*i / dim_model))
...
if __name__ == "__main__":
    ...
    if frase_em_tokens.shape[0] == 2:
        ...
        # 4. POSITIONAL ENCODING (codificação posicional)
        pos_enc = encoding_posicional(compr_seq, dim_model)          # (seq_len, dim_model)
        # Adicionamos a codificação a cada sequência do batch (broadcasting)
        frase_embedding = frase_embedding + pos_enc  
        ...
        print(f'\t- Posicional encoding  {pos_enc.shape} ')
