...
class TensorFoward():
    ...
    def foward(self, tensor:Tensor, valor_entrada:np):
        ...
        #pré computando o rope
        seq_len = valor_entrada.shape[0]
        self.cos, self.sin = self.__ferramentas.precompute_rope_cos_sin(seq_len, tensor.dim_head)
        ...
        for i in range(tensor.num_heads):
            ...
            Q_rot = self.__ferramentas.apply_rotary_emb(Q, self.cos, self.sin)
            K_rot = self.__ferramentas.apply_rotary_emb(K, self.cos, self.sin)
            
            # Cálculo da atenção com Q_rot e K_rot
            scores = Q_rot @ K_rot.T / np.sqrt(tensor.dim_head)
